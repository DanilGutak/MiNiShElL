...
/*   Updated: 2023/11/04 23:02:43 by vfrants          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    do_heredoc(t_data *data, t_cmd_table *cmd_table, int i)
{
        ...
        line = get_next_line(STDIN_FILENO, READ); // returns a string without a \n at the end
        if (line == NULL)
        ...
        while (ft_strcmp(line, stop) != 0)
        {
                ...
                free(line);
                line = get_next_line(STDIN_FILENO, READ); // here as well
                if (line == NULL)
